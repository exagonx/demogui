...
test.ScreenWidth = (128 * 8)
test.ScreenHeight = (96 * 8)
...
'Info Window design
	...
	dim as string InfoRow2 = "Writed by Graziano Falcone in FreeBASIC"
...
dim as integer MainExit = 0

Do	
	if Area = "exit" then 
		if ButtonClose.B_HANDLER() = 1 then MainExit = 1
		...
		if MainMenu.HANDLER() = 1 then 
			...
			if MainMenu.SelectedSubMenu = "E&xit" then MainExit = 1
