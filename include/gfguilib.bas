...
sub GFGUI.AS_CHILD(byval ITEMID as integer = 0)export
	'dim as integer MaxCHILD_ID = 9
	'redim as integer CHILD_ID(0 to 9)
	dim Cycle as integer 
	
	if this.ItemID = 0 then 
		this.ItemID = this.TotalID + 1
		this.TotalID = this.TotalID + 1
	end if
	
	if ( MaxCHILD_ID + 1 ) > this.MaxCHILD_ID then
		
		dim TEMPCHILD_ID(this.TotalID) as integer 'Initialize array for store backup of CHILD_ID(Array)
		
		for Cycle = 0 to this.MaxCHILD_ID step 1 	'----------------
			TEMPCHILD_ID(Cycle) = CHILD_ID(Cycle)	' store data in swap array
		next Cycle									'----------------
		
		redim as integer CHILD_ID(this.MaxCHILD_ID to this.TotalID) 'Redimension CHILD_ID(Array)
		
		for Cycle = 0 to this.MaxCHILD_ID step 1	'----------------
			CHILD_ID(Cycle) = TEMPCHILD_ID(Cycle) 	' recover data in main array
		next Cycle									'----------------
		
	end if
	'CHILD_ID(this.ItemID) = 
end sub
...
function GFGUI.SET_GRAPHICS(byval Cols as integer = - 1, byval Rows as integer = - 1)as integer export

	
	dim as integer ErRet = 0
	...
	if Cols > -1 then 
		...
		Cols = this.ScreenWidth / 8
		if Cols < 1 then ErRet = ErRet + 1
	end if
	
	if Rows > -1 then 
		this.ScreenHeight = Rows * 8 
	else 
		Rows = this.ScreenHeight / 8
		if Rows < 1 then ErRet = ErRet + 1
	end if
